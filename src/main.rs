...
fn create_shim_script(bin_dir: &str) -> Result<()> {
    #[cfg(windows)]
    {
        ...
        let shim_content = concat!(
            "@echo off\r\n",
            "REM Generated by tooler; do not edit (changes are overwritten).\r\n",
            "REM This launcher forwards to \"tooler run <name>\", which resolves the\r\n",
            "REM installed binary and passes all arguments through unchanged. Invoke\r\n",
            "REM the command by name as usual; there is no need to locate the\r\n",
            "REM underlying executable or how it is managed.\r\n",
            "REM To inspect or manage the installation: \"tooler list\",\r\n",
            "REM \"tooler info <name>\", \"tooler update <name>\". See \"tooler --help\".\r\n",
            "set \"tool_name=%~n0\"\r\n",
    ...
    #[cfg(not(windows))]
    {
        ...
        let shim_content = format!(
            r#"#!/bin/bash
# Generated by tooler; do not edit (changes are overwritten).
# This launcher forwards to "tooler run <name>", which resolves the installed
# binary and passes all arguments through unchanged. Invoke the command by
# name as usual; there is no need to locate the underlying executable or how
# it is managed.
# To inspect or manage the installation: "tooler list", "tooler info <name>",
# "tooler update <name>". See "tooler --help".
set -u
...
fn create_tool_symlink(bin_dir: &str, tool_name: &str) -> Result<()> {
    #[cfg(windows)]
    {
        ...
        // Per-tool shims are copies of tooler-shim.cmd; refresh them whenever
        // the master shim content changes so they don't drift across versions.
        let needs_copy = match fs::read_to_string(&symlink_path) {
            Ok(existing) => existing != fs::read_to_string(&shim_path).unwrap_or_default(),
            Err(_) => true,
        };
        if needs_copy {
